...
local function AllowVehicleClass(restrictionData, vehicle)
    ...
    if (restrictionData.allowedClasses and restrictionData.allowedClasses[1] == nil) or not restrictionData.allowedClasses or vehicleClass == 0 then 
        return true 
    end
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("Ayse_Customs:server:attemptPurchase", type, upgradeLevel, CustomsData.location)
...
function EnterLocation(override)
    ...
    if not canEnter then
        lib.notify({
                description = 'You cant do anything here.',
                type = 'inform'
        })
        ExitBennys()
        return
...
function GetLocations()
    lib.callback("Ayse_Customs:server:GetLocations", false, function(locations)
...
function CheckForGhostVehicle()
    ...
    local closestVehicle, closestDistance = lib.getClosestVehicle(pedCoords, 2.5, false)
    if closestVehicle > 0 and #(pedCoords - closestDistance) <= 10.0 and GetEntityCollisionDisabled(closestVehicle) and GetPedInVehicleSeat(closestVehicle, -1) == 0 then
...
function CheckRestrictions(location)
    ...
    return isEnabled and vehicle ~= 0 and allowedClass
end
...
function SetupInteraction()
    ...
    CheckForKeypress()
    lib.showTextUI(text)
    if (Vdist(pos.x, pos.y, pos.z) < 5.5) then
        lib.hideTextUI()
    end
end
...
RegisterNetEvent('Ayse_Customs:client:getLocations', function(source)
    GetLocations()
end)

RegisterNetEvent('Ayse_Customs:client:UpdateLocation', function(location, type, key, value)
    config.locations[location][type][key] = value
end)

RegisterNetEvent("Ayse_Customs:client:purchaseSuccessful", function()
...
RegisterNetEvent("Ayse_Customs:client:purchaseFailed", function()
...
RegisterNetEvent('Ayse_Customs:client:EnterCustoms', function(override)
